...
var SHOULD_COMPILE_TO_BYTECODE = true;
...
var mainPlugins = [swcPlugin(), yaml(), externalizeDepsPlugin()];
var electron_vite_config_default = defineConfig({
  main: {
    plugins: SHOULD_COMPILE_TO_BYTECODE ? [bytecodePlugin({ transformArrowFunctions: false }), ...mainPlugins] : mainPlugins,
    build: {
      minify
    },
    resolve: {
    ...
    build: {
      ...
      rollupOptions: {
        input: {
          ...
          opggWindow: resolve(__electron_vite_injected_dirname, "src/renderer/opgg-window.html"),
          ongoingGameWindow: resolve(__electron_vite_injected_dirname, "src/renderer/ongoing-game-window.html"),
          cdTimerWindow: resolve(__electron_vite_injected_dirname, "src/renderer/cd-timer-window.html")
        }
      }
    }
  }
